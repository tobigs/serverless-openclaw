...
**⚠️ Deploy-order trap:** ApiStack's Lambdas (`telegram-webhook`, `ws-message`, `watchdog`, `prewarm`) bake the task-definition ARN into their environment variables at ApiStack's own deploy time — they do NOT read SSM live at runtime. Deploying `ComputeStack` or `LambdaAgentStack` alone (e.g. to change `AI_MODEL`) replaces the task definition and marks the old revision `INACTIVE`, but ApiStack's Lambdas keep calling the stale ARN until ApiStack itself is redeployed. Symptom: every Telegram/WebSocket message fails with `InvalidParameterException: TaskDefinition is inactive`, which surfaces to users as repeated "waking up" retries that never complete. **Always run `cdk deploy ApiStack` immediately after a standalone `ComputeStack`/`LambdaAgentStack` deploy.** `cdk deploy --all` is unaffected since it deploys in dependency order.
